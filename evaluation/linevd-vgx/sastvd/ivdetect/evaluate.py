...
def helper(row):
    ...
    """
    before_path = str(svd.processed_dir() / f"{row['dataset']}/before/{row['id']}.c")
    after_path = str(svd.processed_dir() / f"{row['dataset']}/after/{row['id']}.c")
    try:
...
def get_dep_add_lines_bigvul(cache=True):
    ...
    for project in [config.get_trainset(), config.get_partset(), config.get_testset()]:
        ...
        saved = svd.get_dir(svd.processed_dir() / f"{project}/eval") / "statement_labels.pkl"
        if os.path.exists(saved) and cache:
            with open(saved, "rb") as f:
                lines = pkl.load(f)
        else:
            lines = get_project(project)
...
def get_project(project):
    ...
    df = svdd.bigvul()
    df = df[df.vul == 1]
    df = df[df.dataset == project]
    desc = "Getting dependent-added lines: "
    lines_dict = svd.dfmp(df, helper, ["id", "removed", "added", "dataset"], ordr=False, desc=desc)
