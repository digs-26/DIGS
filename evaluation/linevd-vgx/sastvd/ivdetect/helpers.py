...
class IVDetect(nn.Module):
    ...
    def forward(self, g, dataset, e_weights=[]):
        ...
        for n in nodes:
            subseq_array = np.array(data[n]["subseq"])
            f1 = torch.Tensor(subseq_array)
            ...
            nametypes_array = np.array(data[n]["nametypes"])
            f3 = torch.Tensor(nametypes_array)
...
class BigVulDatasetIVDetect(svddc.BigVulDataset):
    ...
    def __init__(self, **kwargs):
        ...
        print(f"Shuffled df: {self.df.head()}")

    def item(self, _id):
        """Get item data."""
        dataset_id = self.idx2id[_id]
        n, _ = feature_extraction(svddc.BigVulDataset.itempath(dataset_id))
        ...
        def ast_dgl(row, lineid):
            ...
            # import pdb
            # pdb.set_trace()
            embeddings = np.array(svdg.get_embeddings_list(ndata, self.emb_dict, 200))
            g.ndata["_FEAT"] = torch.Tensor(embeddings)
    ...
    def __getitem__(self, idx):
        ...
        embeddings = np.array(svdg.get_embeddings_list(n["subseq"].to_numpy(), self.emb_dict, 200))
        g.ndata["_FEAT"] = torch.Tensor(embeddings)
