...
def gnnexplainer(model, graph, dataset):
    ...
    # idx = graph.ndata["_SAMPLE"].max().int().item()
    # asts = dataset.item(idx)["asts"]
    # asts = [i for i in asts if i is not None]
    # asts = dgl.batch(asts).to(dev)
    # sub_feats = asts.ndata["_FEAT"]
    ...
    #     reverse=True,
    # )
    if (len(graph.ndata["_LINE"].detach().cpu().numpy()) == 1):
        ret = (graph.ndata["line_importance"].squeeze().detach().cpu().numpy(), graph.ndata["_LINE"].detach().cpu().numpy())
        return [float(graph.ndata["_LINE"].detach().cpu().numpy())]
    else:
        ret = sorted(
            list(
                zip(
