...
class BigVulDataset:
    ...
    def __init__(self, partition="train", vulonly=False, sample=-1, splits="default", df=None):
        """Init class."""
        # Get finished samples
        # self.finished = [int(Path(i).name.split(".")[0]) for i in glob(str(svd.processed_dir() / "bigvul/before/*nodes*"))]
        self.finished = []
        for project in [config.get_trainset(), config.get_partset(), config.get_testset()]:
            self.finished.extend(
                [Path(i).name.split(".")[0] for i in glob(str(svd.processed_dir() / f"{project}/before/*nodes*"))])
        ...
        if partition == "train" or partition == "val":
            vul = self.df[self.df.vul == True]
            nonvul = self.df[self.df.vul == False]#.sample(len(vul), random_state=0)
            self.df = pd.concat([vul, nonvul])

        # Correct ratio for test set
        if partition == "test":
            vul = self.df[self.df.vul == True]
            nonvul = self.df[self.df.vul == False]
            #vul = vul.sample(int(len(nonvul)/18), random_state=0)
        ...
        print(f"Valid samples: {len(self.df)}")
        class_counts = self.df.groupby("dataset").size().reset_index(name="count")
        print(class_counts)

        # Get mapping from index to sample ID.
        self.df = self.df.reset_index(drop=True).reset_index()
        self.df = self.df.rename(columns={"idx": "file_name"})
    ...
    def itempath(_id):
        ...
        if '_' in _id:
            dataset = _id.split("_")[1]
            return svd.processed_dir() / f"{dataset}/before/{_id}.c"
        else:
