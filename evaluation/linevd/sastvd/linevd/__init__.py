...
class BigVulDatasetLineVD(svddc.BigVulDataset):
    ...
    def item(self, _id, codebert=None):
        ...
        if '_' in str(_id):
            ...
            s = sast.get_sast_lines(svd.processed_dir() / f"{dataset}/before/{_id}.c.sast.pkl")
        else:
    ...
    def cache_codebert_method_level(self, codebert):
        ...
        done = [i.split("/")[-1].split(".")[0] for i in glob(str(savedir / "*"))]
        done = set(done)
...
class BigVulDatasetLineVDDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        dataargs = {"sample": sample, "gtype": gtype, "splits": splits, "feat": feat, "df": self.df}
        self.train = BigVulDatasetLineVD(partition="train", **dataargs)
        self.val = BigVulDatasetLineVD(partition="test", **dataargs)
        self.test = BigVulDatasetLineVD(partition="test", **dataargs)
        print("Train:", len(self.train), "Val:", len(self.val), "Test:", len(self.test))
...
class LitGNN(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("train_loss", loss.float(), batch_size=batch_size, on_epoch=True, prog_bar=True, logger=True)
        self.log("train_acc", acc.float(), batch_size=batch_size, prog_bar=True, logger=True)
        if not self.hparams.methodlevel:
            self.log("train_acc_func", acc_func.float(), batch_size=batch_size, prog_bar=True, logger=True)
        self.log("train_mcc", mcc.float(), batch_size=batch_size, prog_bar=True, logger=True)
        return loss
