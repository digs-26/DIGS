...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train", dataset="none"):
        ...
        for i in tqdm(range(len(funcs)), desc=f"load {file_type} dataset"):
            if len(project) != 0:
                self.examples.append(convert_examples_to_features(funcs[i], labels[i], tokenizer, args, indices[i], project[i]))
            else:
                self.examples.append(convert_examples_to_features(funcs[i], labels[i], tokenizer, args, indices[i], dataset))
...
def train(args, train_dataset, model, tokenizer, test1_dataset, test2_dataset, eval_dataset=None):
    """ Train the model """
    # build dataloader
        ...
        for step, batch in enumerate(bar):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                # if global_step == args.max_steps:
                    ...
                    checkpoint_prefix = 'checkpoint-' + str(idx) + '-' + str(args.seed) + '-' + str(args.handlesets)
...
def test_line(args, model, tokenizer, test_dataset, test_index, best_threshold=0.5):
    # build dataloader
    test_sampler = SequentialSampler(test_dataset)
    test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=args.eval_batch_size, num_workers=0)
    ...
    if args.do_local_explanation:
        for reasoning_method in all_reasoning_method:
            ...
            df = pd.read_csv(args.test1_data_file)
            ...
            if not os.path.exists("./ifa_records"):
                os.makedirs("./ifa_records")
...
def main():
    ...
    if args.do_train:
        ...
        save_path = os.path.join("/root/my_eval/RQ1/linevul_storage", f"storage_{train_name}_{part_name}")
        ...
        if args.handlesets != "none":
            ...
            if args.under != 1.0:
                if args.over == "vulfilter":
                    start_select_time = time.time()
                    output_dir = os.path.dirname(args.output_dir)
                    part_dataset.examples = pos_examples
                    vulfilter.get_cls(args, model, train_dataset, part_dataset, save_path)
                    # pdb.set_trace()
                    train_pos, train_neg, part_pos, part_neg = vulfilter.load_cls(save_path)
                    file_name = f"KR{args.KR}_KG{args.KG}"
                    save_path = os.path.join(save_path, file_name)
                    if not os.path.exists(save_path):
                        os.makedirs(save_path)
                        print(f"create {save_path}")
                    vulfilter.selector(train_pos, train_neg, part_pos, part_neg, save_path, output_dir, args.handlesets,
                                       args.under, args.over, args)
                    vulfilter.filter(train_dataset, part_dataset, output_dir, args.handlesets)
                    ...
                    logger.info(f"select time: {end_select_time - start_select_time:.2f}s")
        ...
        if args.eval_data_file != "":
            eval_dataset_file = os.path.join(save_path, 'eval_dataset.pkl')
            if os.path.exists(eval_dataset_file):
                eval_dataset = pickle.load(open(eval_dataset_file, 'rb'))
            else:
                eval_dataset = TextDataset(tokenizer, args, file_type='eval')
                pickle.dump(eval_dataset, open(eval_dataset_file, 'wb'))
        else:
            eval_dataset = None
        if args.test1_data_file != "":
            test1_dataset_file = os.path.join(save_path, 'test1_dataset.pkl')
            if os.path.exists(test1_dataset_file):
                test1_dataset = pickle.load(open(test1_dataset_file, 'rb'))
            else:
                test1_dataset = TextDataset(tokenizer, args, file_type='test1')
                pickle.dump(test1_dataset, open(test1_dataset_file, 'wb'))
        else:
            test1_dataset = None
        if args.test2_data_file != "":
            test2_dataset_file = os.path.join(save_path, 'test2_dataset.pkl')
            if os.path.exists(test2_dataset_file):
                test2_dataset = pickle.load(open(test2_dataset_file, 'rb'))
            else:
                test2_dataset = TextDataset(tokenizer, args, file_type='test2')
                pickle.dump(test2_dataset, open(test2_dataset_file, 'wb'))
        else:
            test2_dataset = None
        # train(args, train_dataset, model, tokenizer, eval_dataset)
        train(args, train_dataset, model, tokenizer, test1_dataset, test2_dataset, eval_dataset)
    # Evaluation
    results = {}
    if args.do_eval:
    ...
    if args.do_test:
        if os.path.exists(args.model_name):
            ...
            # checkpoint_prefix = f'checkpoint-{args.idx}/{args.model_name}'
        ...
        test1_dataset = TextDataset(tokenizer, args, file_type='test1')
        # test2_dataset = TextDataset(tokenizer, args, file_type='test2')
        test_line(args, model, tokenizer, test1_dataset, 1, best_threshold=0.00052)#0.05
        # test(args, model, tokenizer, test2_dataset, 2, best_threshold=0.5)
